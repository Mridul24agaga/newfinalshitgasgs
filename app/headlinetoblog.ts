...
async function generateExternalLinks(
  ...
  // Only try to get domain if websiteUrl is provided
  let domain = ""
  try {
    if (websiteUrl) {
  ...
  // Extract URLs from search results (exclude same domain if domain exists)
  if (searchResults.length > 0) {
    searchResults.forEach((result) => {
      const url = result.url || ""
      const title = result.title || "Related Resource"
      if (url && (!domain || !url.includes(domain)) && !externalLinks.some((link) => link.url === url)) {
        externalLinks.push({ text: title, url })
      }
    })
  }

  // Extract URLs from research summary (exclude same domain if domain exists)
  const urlRegex = /(https?:\/\/[^\s<]+)/g
  const summaryUrls = researchSummary.match(urlRegex) || []
  summaryUrls.forEach((url) => {
    if ((!domain || !url.includes(domain)) && !externalLinks.some((link) => link.url === url)) {
  ...
  if (externalLinks.length < 8) {
    try {
      // If no domain, use generic queries
      const additionalQueries = domain
        ? [
            `${domain} alternatives`,
      ...
      for (const query of additionalQueries) {
        ...
        if (tavilyClient) {
          ...
          console.log("Tavily client not available, skipping search")
        }
      }
    } catch (error) {
...
// Removed findYouTubeVideo function
...
async function generateInternalLinks(
  ...
  try {
    ...
    console.error("Error parsing website URL for internal links:", error)
  }
  ...
  // Extract subpages from search results (same domain) if websiteUrl is provided
  if (websiteUrl && searchResults.length > 0) {
    searchResults.forEach((result) => {
      const url = result.url || ""
...
if (!TAVILY_API_KEY) {
  console.warn("Tavily API key is missing. Some search functionality will be limited.")
}
...
async function extractKeywordsFromHeadline(headline: string): Promise<string[]> {
  console.log(`Extracting keywords from headline: "${headline}"`)

  try {
    const keywordsPrompt = `
      Extract 5-7 important keywords or phrases from this headline that would be useful for researching content:
      "${headline}"
      ...
      3. Action words
      4. Specific metrics or numbers mentioned
      5. Any unique concepts or methodologies
      
      Return only the keywords, one per line, with no numbering or additional text.
    `

    const keywordsResponse = await callAzureOpenAI(keywordsPrompt, 200, 0.7)
    ...
    // Fallback: simple extraction by splitting and filtering
    const simpleKeywords = headline
      .split(/\s+/)
      .filter((word) => word.length > 3)
      .filter(
        (word) =>
          !["with", "that", "this", "from", "your", "will", "have", "what", "when", "where", "which", "about"].includes(
            word.toLowerCase(),
          ),
      )
      .slice(0, 5)

    console.log(`Fallback keywords: ${simpleKeywords.join(", ")}`)
    return simpleKeywords
  }
}

// Perform web searches based on keywords
async function searchWebWithKeywords(keywords: string[], websiteUrl = ""): Promise<any[]> {
  console.log("Performing web searches with keywords...")
  let allResults: any[] = []
  ...
  try {
    ...
    for (const keyword of keywords) {
      try {
        ...
        if (tavilyClient) {
          ...
          if (response.results && response.results.length > 0) {
            console.log(`Found ${response.results.length} results for "${keyword}"`)
    ...
    // If website URL is provided, also search for combined queries
    if (websiteUrl) {
      try {
        const domain = new URL(websiteUrl).hostname

        // Search for each keyword combined with the domain
        for (const keyword of keywords) {
          const combinedQuery = `${keyword} ${domain}`
          console.log(`Searching web for combined query: "${combinedQuery}"`)

          if (tavilyClient) {
            const response = await tavilyClient.search(combinedQuery, {
              max_results: 3,
              search_depth: "basic",
            })

            if (response.results && response.results.length > 0) {
              console.log(`Found ${response.results.length} results for combined query "${combinedQuery}"`)
              allResults = [...allResults, ...response.results]
            }
          } else {
            console.log("Tavily client not available, skipping search")
        ...
        console.log(`Error searching with domain:`, error)
      }
    }
    ...
    console.error("Error in searchWebWithKeywords:", error)
    return []
  }
}
...
async function searchWeb(websiteUrl: string): Promise<any[]> {
  ...
  try {
    // If no website URL, use generic search terms
    if (!websiteUrl) {
      ...
      for (const query of genericQueries) {
        try {
          console.log(`Searching web for generic query: "${query}"`)
          if (tavilyClient) {
            const response = await tavilyClient.search(query, {
              max_results: 5,
              search_depth: "basic",
      ...
      console.log(`Total generic results found: ${allResults.length}`)
      return allResults
    }

    // If website URL is provided, use domain-specific queries
    let domain = ""
    try {
      domain = new URL(websiteUrl).hostname
    } catch (error) {
      console.error("Error parsing website URL for web search:", error)
      return []
    }
...
async function analyzeWebsite(websiteUrl: string): Promise<string> {
  // If no website URL is provided, return generic info
  if (!websiteUrl) {
    return `
Website: Not provided
Domain: Not provided
Name: Not provided
Description: No website URL was provided. This blog post will focus on general industry best practices and trends.
    `
  }
...
async function generateFirstHalf(blogTitle: string, researchSummary: string, websiteUrl: string): Promise<string> {
  ...
  // Handle case when no website URL is provided
  const websiteReference = websiteUrl ? `for ${websiteUrl}` : "on this topic"

  const firstHalfPrompt = `
    I'm a professional content writer working on a blog post ${websiteReference}. My marketing consultant gave me this research:
    ...
    4. ${websiteUrl ? `Two placeholders for internal links ([INTERNAL_LINK_1], [INTERNAL_LINK_2]) to pages on ${websiteUrl}` : "Two placeholders for related content links ([INTERNAL_LINK_1], [INTERNAL_LINK_2])"}
    ...
    Be specific to this ${websiteUrl ? "website and " : ""}industry - don't make up generic information.
...
async function generateSecondHalf(
  ...
  const secondHalfPrompt = `
    I'm continuing to write a blog post ${websiteReference} with the title:
    ...
    5. ${websiteUrl ? `One placeholder for an internal link ([INTERNAL_LINK_3]) to a page on ${websiteUrl}` : "One placeholder for a related content link ([INTERNAL_LINK_3])"}
    ...
    ${websiteUrl ? "Be specific to this website and industry - don't make up generic information." : "Focus on providing valuable industry insights."}
...
// Modify the finalHumanization function to remove YouTube video references
async function finalHumanization(
  ...
  // Removed YouTube video placeholder
  ...
  // Removed YouTube video replacement
...
// Function to blur content for free users
function blurContent(content: string): string {
  // This function would implement the blurring logic
  // For simplicity, we'll just add a note at the beginning
  return `<div class="blur-notice" style="background-color: #fff3cd; border: 1px solid #ffeeba; color: #856404; padding: 15px; margin-bottom: 20px; border-radius: 5px;">
    <p style="margin: 0;"><strong>This content is blurred.</strong> Subscribe to an active plan to view the full content.</p>
  </div>
  <div class="blurred-content" style="filter: blur(5px);">
    ${content}
  </div>`
}

// Main function to generate blog from headline and website URL
export async function generateBlog(websiteUrl = "", headline?: string) {
  try {
    console.log("Starting blog generation with Tavily web search...")
    console.log(`Headline provided: ${headline || "None"}`)
    console.log(`Website URL provided: ${websiteUrl || "None"}`)
    ...
    try {
      ...
      console.log("Will continue despite database error")
    }
    ...
    if (websiteUrl) {
      ...
      try {
        ...
        console.warn(`Invalid URL format: ${websiteUrl}, proceeding without website URL`)
        validatedUrl = ""
      }
    }
    ...
    // If headline is provided, extract keywords and perform targeted searches
    let searchResults = []
    if (headline) {
      console.log(`Performing targeted searches based on headline: "${headline}"`)

      // Extract keywords from headline
      const keywords = await extractKeywordsFromHeadline(headline)

      // Search web with extracted keywords
      searchResults = await searchWebWithKeywords(keywords, validatedUrl)
    } else {
      // If no headline, fall back to the original website search
      searchResults = await searchWeb(validatedUrl)
    }
...
`
      ...
      try {
        // If we have a domain, search for industry info
        if (validatedUrl) {
...
`
          ...
          // If no domain, use generic industry info
          webContent = `
No specific web content found. Using general industry knowledge for content creation.
`
        }
      } catch (error) {
...
`
    ...
    // Modify research prompt to include headline if provided
    const researchPrompt = headline
      ? `
        I'm writing a blog post ${validatedUrl ? `for ${validatedUrl}` : ""} with the headline "${headline}". Here's what I know about the ${validatedUrl ? "website" : "topic"} and some web research:
        
        ${combinedContent}
        
        Based on this information, help me understand:
        1) How does this headline relate to the ${validatedUrl ? "website's purpose" : "topic"}?
        2) What's the main purpose of this ${validatedUrl ? "website/business" : "topic"}?
        3) Who is the target audience?
        4) What are the key ${validatedUrl ? "services or products they offer" : "aspects of this topic"}?
        5) What are the main pain points ${validatedUrl ? "their customers have" : "related to this topic"}?
        6) What specific data points or statistics would be valuable to include in a blog post with this headline?
        7) What are 3-5 key points that should be covered in a blog post with this headline?
        
        Please write this as if you're a marketing consultant giving me advice for my blog post.
        ${validatedUrl ? "Be specific to this website and industry - don't make up generic information." : "Focus on providing valuable insights about the topic."}
        If you don't have enough information about something, acknowledge that gap rather than inventing details.
      `
      : `
        I'm writing a blog post ${validatedUrl ? `for ${validatedUrl}` : "on a general topic"}. Here's what I know about the ${validatedUrl ? "website" : "topic"} and some web research:
        
        ${combinedContent}
        
        Based on this information, help me understand:
        1) What's the main purpose of this ${validatedUrl ? "website/business" : "topic"}?
        2) Who is the target audience?
        3) What are the key ${validatedUrl ? "services or products they offer" : "aspects of this topic"}?
        4) What are the main pain points ${validatedUrl ? "their customers have" : "related to this topic"}?
        5) What are 3-5 trending topics in this industry I could write about?
        6) What are some common questions people have about this ${validatedUrl ? "industry" : "topic"}?
    ...
    // Use provided headline or generate one based on research
    const blogTitle = headline || (await generateDataDrivenTitle(websiteInfo, researchResults))
    console.log(`Using blog title: ${blogTitle}`)

    // Removed YouTube video search
    ...
    try {
      ...
      if (saveError) {
        ...
        console.log("Will continue despite save error")
      } else {
        console.log("Blog post saved successfully to headlinetoblog table")
      }
    } catch (dbError) {
      console.error("Database operation failed:", dbError)
      // Continue execution instead of crashing
    ...
    // Modified return statement to remove youtubeVideo property
    return {
      headline: blogTitle,
    ...
    try {
      ...
      // Try to get domain if website URL is provided
      if (websiteUrl) {
        try {
...
Pricing information for ${domain} would be available on their official website. Many similar services offer tiered pricing models with free trials or freemium options, but specific details would need to be confirmed directly from their pricing page.
      ...
      const fallbackFirstHalfPrompt = `
        ...
        2. The first 2-3 main sections with clear headings
        3. Two placeholders for external links ([EXTERNAL_LINK_1], [EXTERNAL_LINK_2]) to relevant industry resources
        4. Two placeholders for internal links ([INTERNAL_LINK_1], [INTERNAL_LINK_2]) to pages on ${websiteUrl || "your website"}
      ...
      const fallbackSecondHalfPrompt = `
        ...
        3. A strong conclusion with a call-to-action
        4. One placeholder for an external link ([EXTERNAL_LINK_3]) to a relevant industry resource
        5. One placeholder for an internal link ([INTERNAL_LINK_3]) to a page on ${websiteUrl || "your website"}
      ...
      const fallbackInternalLinks = [
        { text: "About Us", url: websiteUrl ? `https://${domain}` : "https://example.com/about" },
        { text: "Pricing Plans", url: websiteUrl ? `https://${domain}/pricing` : "https://example.com/pricing" },
        { text: "Contact Support", url: websiteUrl ? `https://${domain}/contact` : "https://example.com/contact" },
      ]
