...
import { createClient } from "@/utitls/supabase/client"; // Client-side Supabase
...
export default function SettingsPage() {
  ...
  const handleKeywordSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      const { data: updatedKeywords } = await supabase
        .from("keywords_to_be_used")
        .select("*")
        .eq("user_id", user.id);
  ...
  if (loading) {
    return <div className="container mx-auto p-6 text-center">Loading settings...</div>;
  }
        ...
        <form onSubmit={handleKeywordSubmit} className="space-y-6 bg-gray-100 p-6 rounded-lg shadow">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setNewKeyword({ ...newKeyword, priority: parseInt(e.target.value) || 3 })}
              className="mt-1 block w-full border rounded-md p-2 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
                ...
                <li key={keyword.id} className="bg-white p-4 rounded-lg shadow flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Priority: {keyword.priority} | Difficulty: {keyword.difficulty} | Context: {keyword.usage_context || "None"}
                    </p>
                  </div>
                  <button
